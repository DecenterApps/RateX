...
abstract contract UniViewQuoter is IUniswapState, IUniswapViewQuoter {
  ...
  function estimateAmountOut(address _pool, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256) {
    bool zeroForOne = _tokenIn < _tokenOut;
    return
      _quoteSwapExactAmount(
        _pool,
        int256(_amountIn),
  ...
  function _quoteSwapExactAmount(
    ...
    while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != _sqrtPriceLimitX96) {
      ...
      (step.tickNext, step.initialized, step.sqrtPriceNextX96) = _getNextTick(_poolAddress, state.tick, tickSpacing, _zeroForOne);
  ...
  function _getNextTick(
    ...
    (tickNext, initialized) = TickBitmap.nextInitializedTickWithinOneWord(_poolAddress, _tick, _tickSpacing, _zeroForOne);
