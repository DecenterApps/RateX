 ...
 */
async function preprocessPools(
  tokenIn: string,
  tokenOut: string,
  amountIn: bigint,
  chainId: number
): Promise<[AdditionalPoolInfo[], AdditionalPoolInfo[], Map<string, AdditionalPoolInfo>]> {
  // Graph
  ...
  // Divide pools back into poolsIn and poolsOut
  const poolsInAdditionalInfo: AdditionalPoolInfo[] = topPoolsIn
    .map((pool) => poolsAdditionalInfoMap.get(pool.poolId))
    .filter((info) => info !== undefined) as AdditionalPoolInfo[]
  const poolsOutAdditionalInfo: AdditionalPoolInfo[] = topPoolsOut
    .map((pool) => poolsAdditionalInfoMap.get(pool.poolId))
    .filter((info) => info !== undefined) as AdditionalPoolInfo[]
...
function findViableRoutes(poolsIn: AdditionalPoolInfo[], poolsOut: AdditionalPoolInfo[]): Route[] {
  return poolsIn.flatMap((poolIn) =>
    poolsOut.filter((poolOut) => poolIn.tokenA === poolOut.tokenB).map((poolOut) => ({ pools: [poolIn.poolId, poolOut.poolId] }))
  )
}
...
function calculateExpectedOutputAmountRoute(poolInfoMap: Map<string, AdditionalPoolInfo>, route: Route, amountIn: bigint): bigint {
  ...
  for (let i = 0; i < route.pools.length; i++) {
    ...
    if (pool) amountOut = calculateExpectedOutputAmountPool(amountOut, pool.reserveA, pool.reserveB)
  }
  return amountOut
}
...
function aggregateFoundRoutesForSolidity(routes: Route[]): Map<Route, number> {
  ...
  for (let i = 0; i < routes.length; i++) {
    if (routeToPercentageMap.has(routes[i])) routeToPercentageMap.set(routes[i], routeToPercentageMap.get(routes[i])! + SWAP_PERCENT)
    else routeToPercentageMap.set(routes[i], SWAP_PERCENT)
  }
